...
#' \code{pooled_lfc} is intended to be a matrix of pooled log fold change ratios that map
#' to known interaction modes
...
#' @param pooled_modes character vector representing interaction mode classifications
#' pooled from n signal combinations
#' @param pca_scores matrix of PCA scores generated from princomp()
#' @return list with training scores, training modes, testing scores, and testing modes
#' @examples
#' \dontrun{
#' #FINISH LATER
#' }
...
#' train k-nearest neighbours model with pca scores as feature vectors and interaction modes
#' as classifications
#'
#' @importFrom caret trainControl train
#' @param my_train_scores matrix where each row represents a feature vector (gene)
#' @param my_train_modes character vector of length n where each element indicates the
#' interaction mode classification for a gene
